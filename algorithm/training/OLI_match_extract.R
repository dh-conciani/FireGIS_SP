...
raster_loc <- list.files(path = './train_scenes/s_barb/OLI', pattern = '.tif$', full.names = T)
...
BA_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  ## build data.frame
  BA_OLI_data_entries <- rbind (BA_OLI_data_entries, temp)
}
...
# Create a data.frame to receive data
CONST_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, class, sensor, temp)
  ## build data.frame
  CONST_OLI_data_entries <- rbind (CONST_OLI_data_entries, temp)
}
...
# Create a data.frame to receive data
ESOIL_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, class, sensor, temp)
  ## build data.frame
  ESOIL_OLI_data_entries <- rbind (ESOIL_OLI_data_entries, temp)
}
...
# Create a data.frame to receive data
GCOVER_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, class, sensor, temp)
  ## build data.frame
  GCOVER_OLI_data_entries <- rbind (GCOVER_OLI_data_entries, temp)
}
...
# Create a data.frame to receive data
HARV_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, class, sensor, temp)
  ## build data.frame
  HARV_OLI_data_entries <- rbind (HARV_OLI_data_entries, temp)
}
...
# Create a data.frame to receive data
ROAD_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, class, sensor, temp)
  ## build data.frame
  ROAD_OLI_data_entries <- rbind (ROAD_OLI_data_entries, temp)
}
...
# Create a data.frame to receive data
SHADOW_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, class, sensor, temp)
  ## build data.frame
  SHADOW_OLI_data_entries <- rbind (SHADOW_OLI_data_entries, temp)
}
...
# Create a data.frame to receive data
WATER_OLI_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, class, sensor, temp)
  ## build data.frame
  WATER_OLI_data_entries <- rbind (WATER_OLI_data_entries, temp)
}

## merge 
df_OLI <- rbind (BA_OLI_data_entries, CONST_OLI_data_entries, ESOIL_OLI_data_entries,
                 GCOVER_OLI_data_entries, HARV_OLI_data_entries, ROAD_OLI_data_entries,
                 SHADOW_OLI_data_entries, WATER_OLI_data_entries)
...
rm (df, df_poly, df_raster, poly_dates, poly_list_count, poly_list_names, 
    ...
    r_test, pol_ref, BA_OLI_data_entries, CONST_OLI_data_entries, df_ba, df_const,
    df_esoil, df_gcover, df_harv, df_road, df_shadow, df_water, ESOIL_OLI_data_entries,
    GCOVER_OLI_data_entries, HARV_OLI_data_entries, match_ba, match_const, match_esoil,
    match_gcover, match_harv, match_road, match_shadow, match_water, ROAD_OLI_data_entries,
    SHADOW_OLI_data_entries, WATER_OLI_data_entries, ba_dates, ba_list_names,
